...
INTERFACE guc_InterfaceDetection = INTERFACE_USART1;
...
void APP_Bootloader_Init(void)
{
    while (1)
    {
        ...
        if (READ_BIT(USART1->SR, USART_SR_RXNE)) // 数据到达
        {
            uint8_t rx = (uint8_t)(USART1->DR & 0xFF);
            if (rx == 0x7F)
            {
...
void APP_Bootloader_ProtocolDetection(void)
{
    ...
    APP_USART_ReadData(USART1, ucDataBuffer, 2);
...
ErrorStatus APP_ExtendedErase(void)
{
    ...
    else
    {
        ...
        uint16_t addr_list[ucDataLength];
        for (i = 0; i < ucDataLength + 1; i++)
        {
            addr_list[i] = (ucDataBuffer[2 * i] << 8) | ucDataBuffer[2 * i + 1];
        }
