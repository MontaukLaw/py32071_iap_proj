...
#ifdef CHECK_SDK
            if (((FLASH->SDKR) & 0x1F) * 0x800 <= dwAddr && dwAddr < (((FLASH->SDKR >> 8) & 0x1F) + 1) * 0x800)
            {
                eResultFlag = ERROR;
                break;
            }
...
#ifdef CHECK_SDK
        if (((FLASH->SDKR) & 0x1F) * 0x800 <= (FLASH_BASE + pwDataBuf[i] * 0x1000) && (FLASH_BASE + pwDataBuf[i] * 0x1000) < (((FLASH->SDKR >> 8) & 0x1F) + 1) * 0x800)
        {
            eResultFlag = ERROR;
            break;
        }
